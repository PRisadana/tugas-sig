<x-layouts.app>
    <div class="p-6">
        <h1 class="text-2xl font-bold text-white mb-4">Edit Ruas Jalan</h1>

        <div class="flex flex-wrap -mx-2">
            
            <!-- FORM -->
            <div class="w-full md:w-1/2 px-2 mb-4">
                <form action="{{ route('ruasjalan.update', $ruasjalan['id']) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-2">
                        ...
                        <select id="provinsi" name="provinsi_id" class="w-full border rounded px-3 py-2">
                            <option value="">Pilih Provinsi</option>
                            @foreach($provinsi as $p)
                                <option value="{{ $p['id'] }}" {{ $p['id'] == $provId ? 'selected' : '' }}>{{ $p['provinsi'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select id="kabupaten" name="kabupaten_id" class="w-full border rounded px-3 py-2">
                            <option value="">Pilih Kabupaten</option>
                            @foreach($kabupaten as $k)
                                <option value="{{ $k['id'] }}" {{ $k['id'] == $kabId ? 'selected' : '' }}>{{ $k['kabupaten'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select id="kecamatan" name="kecamatan_id" class="w-full border rounded px-3 py-2">
                            <option value="">Pilih Kecamatan</option>
                            @foreach($kecamatan as $kc)
                                <option value="{{ $kc['id'] }}" {{ $kc['id'] == $kecId ? 'selected' : '' }}>{{ $kc['kecamatan'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select name="desa_id" id="desa" class="w-full border rounded px-3 py-2">
                            <option value="">Pilih Desa</option>
                            @foreach($desa as $d)
                                <option value="{{ $d['id'] }}" {{ $d['id'] == $ruasjalan['desa_id'] ? 'selected' : '' }}>{{ $d['desa'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text" name="kode_ruas" value="{{ $ruasjalan['kode_ruas'] }}" class="w-full border rounded px-3 py-2" required>
                    </div>

                    <!-- Nama Ruas -->
                    <div class="mb-2">
                        <label class="block font-semibold text-white">Nama Ruas</label>
                        <input type="text" name="nama_ruas" value="{{ $ruasjalan['nama_ruas'] }}" class="w-full border rounded px-3 py-2" required>
                    </div>

                    <!-- Panjang -->
                    <div class="mb-2">
                        <label class="block font-semibold text-white">Panjang (m)</label>
                        <input type="number" name="panjang" value="{{ $ruasjalan['panjang'] }}" class="w-full border rounded px-3 py-2" required readonly>
                    </div>

                    <!-- Lebar -->
                    <div class="mb-2">
                        <label class="block font-semibold text-white">Lebar (m)</label>
                        <input type="number" name="lebar" value="{{ $ruasjalan['lebar'] }}" class="w-full border rounded px-3 py-2" required>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select name="eksisting_id" class="w-full border rounded px-3 py-2">
                            @foreach($eksisting as $e)
                                <option value="{{ $e['id'] }}" {{ $e['id'] == $ruasjalan['eksisting_id'] ? 'selected' : '' }}>{{ $e['eksisting'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select name="jenisjalan_id" class="w-full border rounded px-3 py-2">
                            @foreach($jenisjalan as $j)
                                <option value="{{ $j['id'] }}" {{ $j['id'] == $ruasjalan['jenisjalan_id'] ? 'selected' : '' }}>{{ $j['jenisjalan'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <select name="kondisi_id" class="w-full border rounded px-3 py-2">
                            @foreach($kondisi as $k)
                                <option value="{{ $k['id'] }}" {{ $k['id'] == $ruasjalan['kondisi_id'] ? 'selected' : '' }}>{{ $k['kondisi'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <textarea name="keterangan" class="w-full border rounded px-3 py-2">{{ $ruasjalan['keterangan'] }}</textarea>
                    </div>

                    <!-- Paths -->
                    <input type="hidden" name="paths" id="paths" value="{{ $ruasjalan['paths'] }}"/>
